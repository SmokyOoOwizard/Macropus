...
namespace Macropus.Database.Impl;
...
public class DatabasesProvider : IDatabasesProvider
{
	...
	{
		var dbPath = Path.Combine(path, ProjectPaths.DATABASES_PROVIDER_DB_NAME);

		var dbProvider =
			new SqliteDbProvider($"Data Source={dbPath}", ProjectPaths.DATABASES_PROVIDER_DB_NAME, Guid.Empty);
