...
internal class FileSystemProvider : IFileSystemProvider
{
    ...
    public async Task<IFileProvider> GetFileAsync(Guid id, FileAccess access, FileShare share,
    ...
    private static async Task<FileSystemDbContext> GetOrCreateDbContextAsync(string path,
        CancellationToken cancellationToken = default)
    {
        ...
        using (var dbConnection = dbProvider.CreateConnection())
        {
            ...
            if (DbUtils.GetTableCount(dbConnection) == 0)
                await DbUtils.MigrateDb<FileSystemDbMigrationsProvider>(dbConnection, 0,
                    FileSystemDbMigrationsProvider.LastVersion, cancellationToken).ConfigureAwait(false);
        }
